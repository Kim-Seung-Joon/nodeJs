...
function connectDB() {
  ...
  MongoClient.connect(databaseUrl, function(err, db) {
    ...
    // database 변수에 할당
    // mongodb3.0 이상 버전에서는 connection을 할때 db명을 명시해야된다.
    // 그래서 db.db('db명') 이런식으로 해줌!!
  ...
  users.insertMany(
    ...
    (err, result) => {
      ...
      if (result.insertedCount > 0) {
        console.log("사용자 기록 추가됨 : " + result.insertedCount);
      } else {
        console.log("추가된 레코드 없습니다!!");
      }
...
// 사용자를 추가하는 라우팅 함수 - client단(login.html)에서 온 데이터를 디비에 추가함
router.route("/process/addUser").post((req, res) => {
  ...
  console.log(
    "요청한 파라미터 : " + paramId + ", " + paramPassword + "," + paramName
  );

  //디비 객체가 초기화된 경우, addUser 함수를 호출하여 사용자를 추가한다.
  if (database) {
    addUser(database, paramId, paramPassword, paramName, (err, result) => {
      ...
      // 결과 객체를 확인후 추가된 데이터가 있다면 아래의 코드를 실행하고
